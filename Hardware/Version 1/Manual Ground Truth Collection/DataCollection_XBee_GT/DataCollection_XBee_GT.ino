...
int bufferIndex = 0;
int sampleCount = 0;
int getGroundTruth = 0;
int groundTruth = 0;
int groundTruthDir = 0;
int shutdown = 0;
long writeCount = 0;
long timestamp = -1;
    ...
    if (xbee.getResponse().isError()){
      ...
      if (xbee.getResponse().getApiId() == RX_16_RESPONSE) {
        ...
        if (DEBUG == 1){
          ...
          } else if (rx16.getData(0) == 101){
            // end
            SerialUSB.println("end");
            if (working){
              SDCloseFile();
            }
          } else if (rx16.getData(0) == 108){ // 'l'
            SerialUSB.println("GT");
            groundTruth = rx16.getData(1);
            groundTruthDir = 1;
            getGroundTruth = 1;
          }else if (rx16.getData(0) == 114){
            SerialUSB.println("GT");
            groundTruth = rx16.getData(1);
            groundTruthDir = 2;
            getGroundTruth = 1;
...
void readData() {
  if (working) {
    ...
    // check ground truth recording
    if (getGroundTruth == 1){
      commandBlock();
      checkBlock();
      fillBlockCommand(groundTruthDir, groundTruth-48);
      SerialUSB.println(groundTruthDir);
      SerialUSB.println(groundTruth);
      checkBlock();
      getGroundTruth = 0;
    }
    fillBlock(amplitude);
...
void fillBlockCommand(int data1, int data2){
  if (flip) {
    buffer1[bufferIndex] = data1 & 0x00FF;
    bufferIndex += 1;
    buffer1[bufferIndex] = data2 & 0x00FF;
    bufferIndex += 1;
  } else {
    buffer2[bufferIndex] = data1 & 0x00FF;
    bufferIndex += 1;
    buffer2[bufferIndex] = data2 & 0x00FF;
    bufferIndex += 1;
  }
}

void fillBlock(int data){
  if (flip) {
    buffer1[bufferIndex] = data & 0x00FF;
    bufferIndex += 1;
    data >>= 8;
    buffer1[bufferIndex] = data;
    bufferIndex += 1;
  } else {
    buffer2[bufferIndex] = data & 0x00FF;
    bufferIndex += 1;
    data >>= 8;
