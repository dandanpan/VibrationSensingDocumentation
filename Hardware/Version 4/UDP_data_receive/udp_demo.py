...
def all_receive_data(sensor_ip_list, time_lim):
    ...
    filter_data = [[0*x for x in range(600*PLOT_NUM)] for j in range(len(sensor_ip_list))]
...
#plot_data[data_location_num] = plot_data[data_location_num] + use_data
	...
	if plot_count[data_location_num] >= PLOT_COUNT:
            ...
            if tmp_len > 0:
                ...
                plt.pause(0.001)     
   ''' 
                filtered_sig = scipy.signal.wiener(plot_data[data_location_num], 501)
                plt.figure(data_location_num +1)
                plt.clf()
                plt.ylim([0, 45000])
