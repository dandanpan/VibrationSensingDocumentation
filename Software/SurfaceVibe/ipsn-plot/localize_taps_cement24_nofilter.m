...
for velocity = 16000 
    ...
    for bandIdx = 14%[14 20] %30-->20
        bFilter = BandPassFilter(25000,20,60,2);

        renderer = SurfaceRenderer(s);
        renderer.plot();
        estErr = [];
        load('data/drive/dataset/wood24-tap.mat');
        for tapIdx = 0:15
            tapIdx
            tdoas = [];
            points = [];
...
%                end
               if onePoint(1) <= 0 && onePoint(2) <= 0
                   continue;
               elseif onePoint(1) <= 0.05 || onePoint(2) >= 39.5
                   continue;
               elseif onePoint(2) <= 0.05 || onePoint(1) >= 39.5
                   continue;
               elseif onePoint(1) >= 39.5 || onePoint(2) >= 39.5
                   continue;
               end
...
save('cement24-unfiltered-tap-results.mat','evaluation');
