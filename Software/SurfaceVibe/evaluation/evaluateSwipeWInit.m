function [ evaluation ] = evaluateSwipeWInit( swipePoints, initPoint, expectedLine, surface, pointNumThreshold )
...
%   5) the perpendicular distances of each point from the expected line
    ...
    for pIdx = 1:swipePointsNum-(pointNumThreshold-1) % 4 for threshold of 5
        testedStd = std(jumpDist(pIdx:pIdx+(pointNumThreshold-1)));
        if testedStd < maxStd
            maxStd = testedStd;
            idealIdx = pIdx;
        end
    ...
    refDir = sign(x1-x2);
    detDir = sign(projPoints(idealIdx,1)-projPoints(idealIdx+(pointNumThreshold-1),1));
    if refDir ~= detDir
    ...
    % if init point is consistant with the detected swipe, use that to
    % re-evaluate the signal
    if ~isempty(initPoint)
        distToInit = distancesFromLine(initPoint, [p1;p2]);
        if distToInit < 1
            % check distance to x1 and x2
            if abs(expectedAngle) == 90
                projInit = ProjectPoint([x1 y1;x2 y2],[initPoint(2), initPoint(1)]);
            else
                projInit = ProjectPoint([x1 y1;x2 y2],initPoint);
            end

            dist1 = distPoints(projInit, [x1,y1]);
            dist2 = distPoints(projInit, [x2,y2]);
            ...
            if dist2 > dist1
                ...
                brob = robustfit(newX,newY);

                x1New = min(newX); x2New = max(newX);
                y1New = brob(2) * x1New + brob(1);
                y2New = brob(2) * x2New + brob(1);

                newEnd = ProjectPoint([x1New y1New;x2New y2New],[x1,y1]);
                x1 = newEnd(1); y1 = newEnd(2);
                newEnd = ProjectPoint([x1New y1New;x2New y2New],[x2,y2]);
                x2 = newEnd(1); y2 = newEnd(2);
                if abs(expectedAngle) == 90 
                    projInit = ProjectPoint([x1New y1New;x2New y2New],[initPoint(2),initPoint(1)]);
                else
                    projInit = ProjectPoint([x1New y1New;x2New y2New],initPoint);
                end
                projPoints = [projInit; projPoints];
                % get points for length calculation
                [~,end1] = min(projPoints(:,1));
                [~,end2] = max(projPoints(:,1));
