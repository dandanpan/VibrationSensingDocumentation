...
% directory = 'data/cement';
% filename = 'cement-test00000.txt';
...
% end
% 
% save('cement-test.mat','events','d','Fs');

load('cement-test.mat');
selectedFrq = 312.5;
...
%             lagDiff = lag(I);
        ...
        if sum(energy(idx,:)) < 0.005 || min(conf(idx,:)) < 0.3
...
%     figure(g);
    ...
    span=10;
%     stdoas(:,2) = smooth(t(:,1),span);
%     stdoas(:,3) = smooth(t(:,2),span);
%     stdoas(:,4) = smooth(t(:,3),span);
    stdoas(:,2) = t(:,1);
    stdoas(:,3) = t(:,2);
    stdoas(:,4) = t(:,3);
