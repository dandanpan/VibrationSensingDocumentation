...
%             figure; plot(clusterCharacter{personID, clusterID});
            ...
            clusterWhiteSig = [clusterWhiteSig; stepSigWhite];
...
for clusterID1 = 1 : numCluster
    for clusterID2 = clusterID1 + 1 : numCluster
        ...
        similarity = 1-sum(differenceBetween.*differenceBetween);
        relationRecorder = [relationRecorder; clusterID1, clusterID2, similarity, clusterInfoPerson(clusterID1),clusterInfoPerson(clusterID2)];
    end
end
[soredV, soredI]= sort(relationRecorder(:,3),'descend');
orderedRelation = relationRecorder(soredI,:);
                    ...
                    for i = 1 : numPeople
                        for j = 1 : size(clusterCharacter, 2)
                            if length(clusterCharacter{i,j}) > 0
                                ...
                                tempDiff = entropy(Y) - entropy(clusterCharacter{i,j});
                                matchMaker = [matchMaker; i,j,tempMatch, tempDiff];
                            end
                        end
                    end
...
%                     fprintf(['personID = ' num2str(personID) '========']);
%                     selectedClusters
