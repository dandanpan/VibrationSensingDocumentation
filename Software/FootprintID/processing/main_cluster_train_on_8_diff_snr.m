...
crossValid = 1;
snrValue = 2;
resultsSummary{snrValue} = [];
save('results_8_snr.mat','resultsSummary','crossValid','snrValue');
            ...
            for personID = 1 : numPeople
                ...
                for speedID = speedSequence(trainingSpeedID)
                    ...
                    for traceID = trainingTraceID
                        ...
                        [ stepEventValue ,stepEventsIdx ] = findpeaks(traceSigFilter,'MinPeakDistance',200,'MinPeakHeight',20,'Annotate','extents');
                        ...
                        [ selectedSteps ] = stepSelectionSNR( traceSigFilter, stepEventsIdx, WIN1, WIN2, snrValue );
...
%                 end
%                 traceHistTrain = [traceHistTrain; tempHist./sum(tempHist)];
%         %         tracePatternTrain = [tracePatternTrain; info.stepFrequencySet(stepIdxs(1),:),tempHist./sum(tempHist)];
%                 tracePatternTrain = [tracePatternTrain; info.stepFrequencySet(stepIdxs(1),1)*length(tempHist),tempHist>0];
%                 tracePatternLabel = [tracePatternLabel; personID];
%             end
%             personHist{personID} = mean(tracePatternTrain(tracePatternLabel == personID,end-clusterCount+1:end));
%             personFreq{personID} = mean(tracePatternTrain(tracePatternLabel == personID,1));
%             personHist{personID} = personHist{personID}./sum(personHist{personID});
%         end
%         % SVM TRAIN
%         svmstruct = svmtrain(tracePatternLabel, tracePatternTrain, '-s 0 -t 2 -b 1 -c 0.5');
% 
%         %% testing two phase
%         traceGT = [];
%         traceEst = [];
...
%                         testingTracePattern = [testingTracePattern; mean(stepFrequency)*length(traceHist), traceHist>0];
...
%                             fz = fz(fm > 0);
%                             fm = fm(fm > 0);
%                             freqDiff = abs(personFreq{i} - mean(stepFrequency));
%                             distPeople(i) = 1/2*sum(fz.^2./fm)+sqrt(freqDiff);
%         %                     fm = personHist{i}>0;
%         %                     fz = traceHist>0;
...
% 
%                         %% dist to each training case
%     %                     distPeople = zeros(1,size(traceHistTrain,1));
%     %                     for i = 1 : size(traceHistTrain,1)
%     %                         fm = traceHistTrain(i,:) + traceHist;
%     %                         fz = traceHistTrain(i,:) - traceHist;
%     %                         fz = fz(fm > 0);
%     %                         fm = fm(fm > 0);
%     %                         freqDiff = abs(personFreq{tracePatternLabel(i)} - mean(stepFrequency));
%     %                         distPeople(i) = 1/2*sum(fz.^2./fm) + sqrt(freqDiff);
%     %     %                     fm = personHist{i}>0;
%     %     %                     fz = traceHist>0;
%     %     %                     distPeople(i) = sum((fm-fz).^2);
...
%         [predicted_label, accuracySVM, decision_values] = svmpredict(testingTraceLabel, testingTracePattern, svmstruct,'-b 1');
        ...
        if analyzeResultFlag == 1
            ...
            traceResult = [];
            for i = 1 : numPeople
                for j = speedSequence(testingSpeedID)
    ...
    %                 end
        ...
        %     predicted_label 
        %     decision_values
%             newLabel1 = traceEst.*(traceDist<=0.31);
%             newLabel2 = traceResult(:,4)'.*(traceDist>0.31);
%             newLabel = newLabel1 + newLabel2;
%             acc3 = sum(newLabel==traceGT)./length(traceGT)
% 
%             newLabel3 = predicted_label.*(max(decision_values,[],2)>0.5);
%             newLabel4 = traceResult(:,4).*(max(decision_values,[],2)<=0.5);
%             newLabel = newLabel3 + newLabel4;
%             acc4 = sum(newLabel==traceGT')./length(traceGT)

            resultsSummary{snrValue} = [resultsSummary{snrValue}; allP1, allP2, allPS, allPT];
            save('results_8_snr.mat','resultsSummary','crossValid','snrValue');
        end
    end
    resultsSummary{snrValue}
...
snrResults = [];
for i = 2 : 6
    snrResults = [snrResults; mean(resultsSummary{i})];
end
figure; 
plot([2:6],snrResults(:,1));hold on;
plot([2:6],snrResults(:,2));hold off;
