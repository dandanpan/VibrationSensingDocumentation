...
traceResultAll = [];
crossValid = 1;
save('results_sepa_all.mat','resultsSummary','crossValid','trainingResultAll','traceResultAll');

for crossValid = 1 : 4
    save('results_sepa_all.mat','resultsSummary','crossValid','trainingResultAll','traceResultAll');
    clear
    load('results_sepa_all.mat');
    ...
    if trainingFlag == 1
        ...
        for personID = 1 : numPeople
            ...
            %% train on all speed
            for speedID = speedSequence(1:7) %1 : numSpeed
    ...
    %                     stepFrequencySet = [stepFrequencySet; stepFrequencyMean];
                ...
                personModel{personID, speedCount}.stepSigs = stepSigs;
                personModel{personID, speedCount}.speedIDLabel = speedIDLabel;
                personModel{personID, speedCount}.traceIDLabel = traceIDLabel;
                personModel{personID, speedCount}.stepIdxLabel = stepIdxLabel;
                personModel{personID, speedCount}.stepFrequencySet = mean(stepFrequency);

                % end of a person's training data
                [clusters] = stepSelection( stepSigs, 0); 
                personModel{personID, speedCount}.clusters = clusters;
...
%                     end
                    ...
                    clusterCharacter{personID, speedCount, clusterID} = mean(stepFCluster);
                end
            end 
        end
        % find frequency ave
        ...
        for speedID = 1:7
            tempSpeed = 0;
            for personID = 1 : numPeople
                tempSpeed = tempSpeed + personModel{personID, speedID}.stepFrequencySet; 
            end
            aveSpeedCrossPeople = [aveSpeedCrossPeople, tempSpeed/numPeople];
        end
    end
    aveSpeedCrossPeople
        ...
        for personID = 1 : numPeople
            ...
            for speedID = speedSequence
                ...
                for traceID = testingTraceID
                    ...
                    speedDiff = abs(aveSpeedCrossPeople - stepFrequencyMean);
                    [~, speedIDDetect] = min(speedDiff);
                    speedIDDetect
                    % filter out-of-range steps
                    stepEventValue = stepEventValue(stepEventsIdx > WIN1 & stepEventsIdx < length(traceSigFilter)-WIN2);
                    stepEventsIdx = stepEventsIdx(stepEventsIdx > WIN1 & stepEventsIdx < length(traceSigFilter)-WIN2);
                    ...
                    for stepID = 1 : length(stepEventsIdx)
                        ...
                        matchMaker = [];
                        for i = 1 : numPeople
                            for k = max(speedIDDetect-1,1):min(speedIDDetect+1,7)
                                for j = 1 : size(clusterCharacter, 3)
                                    if length(clusterCharacter{i,speedIDDetect,j}) > 0
        %                                 tempMatch = max(xcorr(Y,clusterCharacter{i,j}));
                                        tempMatch = 1 - sum((Y'-clusterCharacter{i,speedIDDetect,j}).*(Y'-clusterCharacter{i,speedIDDetect,j}));
