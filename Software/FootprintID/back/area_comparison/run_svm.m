function [ x_tr,y_tr, pred_multi, y_te, speed_te, acc_nosemi_novoting, acc_nosemi_voting, acc_semi_novoting, acc_semi_voting,confi, pred_table] = run_svm( trace_random, ind_speed_tr,  ind_speed_te, semi_layer, x_old, y_old, down_set)
%UNTITLED4 Summary of this function goes here
%   Detailed explanation goes here

load steps_10p_8s.mat
load direction_info.mat
...
[directionInfo, stepInfoAll,...
    traceIDLabel, speedIDLabel, stepPattern, personIDLabel]...
    = preprocess_down(directionInfo, stepInfoAll,...
    traceIDLabel, speedIDLabel, stepPattern, personIDLabel, down_set);
...
num_trace = size(y_pred,1)/num_step;

y_conf = zeros(size(y_pred));

for i = 1:num_trace
    tmp = y_pred(num_step*(i-1)+1:num_step*i);
    y_pred(num_step*(i-1)+1:num_step*i) = mode(tmp);
    y_conf(num_step*(i-1)+1:num_step*i) = sum(tmp== mode(tmp));
end
...
num_trace = size(pred_multi,1)/num_step;
y_conf = zeros(size(pred_multi));

for i = 1:num_trace
    tmp = pred_multi(num_step*(i-1)+1:num_step*i);
    pred_multi(num_step*(i-1)+1:num_step*i) = mode(tmp);
    y_conf(num_step*(i-1)+1:num_step*i) = sum(tmp== mode(tmp));
end
