function [list, num_list] = confi_trace1(data_name, cutoff)

addpath('../libsvm-3.22/matlab/')

load(data_name)
%% CHANGE
ind_rm = union(find(speed_te>=(7-cutoff+1)), find(speed_te<=1+cutoff-1));
...
for i = 1:size(y_conf,1)/7
    ind_start = (i-1)*7+1;
    ind_end = (i-1)*7+7;
    y_pred_trace(i) = mode(y_pred(ind_start:ind_end));
    y_te_trace(i) = mode(y_te(ind_start:ind_end));
