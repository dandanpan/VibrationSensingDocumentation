...
for jj = 1:12
    ...
    for i = 1:10
        if i<10
            load(['bias3_' num2str(i) '  ' num2str(jj) '.mat']);
        elseif jj<10
            load(['bias3_' num2str(i) '   ' num2str(jj) '.mat']);
        else
            load(['bias3_' num2str(i) '  ' num2str(jj) '.mat']);
        ...
        count_hist =  histc(y_te,[1:10]);
        ind_high = find(count_hist == max(count_hist))';
        ind_low = find(count_hist == min(count_hist))';
        
        ind_high_data = ismember(y_te,ind_high);
        ind_low_data = ismember(y_te,ind_low);
        
        acc_low = sum(y_te(ind_low_data)==y_pred(ind_low_data))/size(y_te(ind_low_data),1);
        acc_high = sum(y_te(ind_high_data)==y_pred(ind_high_data))/size(y_te(ind_high_data),1);
